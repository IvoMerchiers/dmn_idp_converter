...
class MetaLanguageConverter(GeneralConverter):
    ...
    def build_meta_input_rule(self, labels: list, rule_components: list) -> list:
        """
    ...
    def __build_single_input_rule(self, rule_nr: int, label: str, comparator: str, value: str) -> [str]:
        ...
        if comparator.startswith(('[', ']')):
            ...
            # recursive call
            first_rule_comp = self.__build_single_input_rule(rule_nr, label, first_dict[inclusion_symbols[0]], values[0])
            second_rule_comp = self.__build_single_input_rule(rule_nr, label, last_dict[inclusion_symbols[1]], values[1])
